...
<!-- <div class="nk-fmg-body-content"> -->
    <div class="nk-fmg-quick-list nk-block">
        <div class="card">
            <div class="card-body">
            <form class="forminput" method="POST" action="{{ url('crud3') }}/{{ $data['id_diklat'] }}">
            @csrf
            @method('PUT')
            <div class="form-group">
                    <label for="no_urut">No Urut</label>
                    <input type="text" class="form-control" id="no_urut" placeholder="No Urut" name="no_urut" value="{{ $data['no_urut'] }}">
                </div>
                <div class="form-group">
                    <label for="nama_diklat">Nama Diklat</label>
                    <input type="text" class="form-control" id="nama_diklat" placeholder="Nama Diklat" name="nama_diklat" value="{{ $data['nama_diklat'] }}">
                </div>
                <div class="form-group">
                <label for="jenis_diklat_id">Jenis Diklat</label>
                <select class="form-control" name="jenis_diklat_id" value="{{ $data['jenis_diklat_id'] }}">
                    @foreach($m_jenis_diklat as $a)
                    <option value= "{{ $a->jenis_diklat_id }}" {{ $data->jenis_diklat_id == $a->jenis_diklat_id ? 'selected' : '' }}>{{ $a->nama_jenis_diklat }}</option>
                    @endforeach
                </div>
                <div class="form-grup">
